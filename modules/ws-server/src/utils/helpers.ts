...
export function deepClone<T>(obj: T): T {
    // Use a WeakMap to track references and handle circular structures
    function _deepClone(source: any, refs: WeakMap<any, any>): any {
        ...
        if (Array.isArray(source)) {
            ...
            for (const item of source) {
                arr.push(_deepClone(item, refs));
            }
            return arr;
        }

        const clonedObj: any = {};
        refs.set(source, clonedObj);
        for (const key of Object.keys(source)) {
            clonedObj[key] = _deepClone(source[key], refs);
        }

        return clonedObj;
    }

    return _deepClone(obj, new WeakMap()) as T;
}
